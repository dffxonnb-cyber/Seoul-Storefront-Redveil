...
| 대상 | 서울 소형 상가 매입을 검토하는 개인 투자자 |
| 방식 | 공공데이터 수집, 리스크 스코어링, 매물 검토 UI, 공개 웹사이트 |
| 범위 | 서울 25개 구, 행정동 428개, 수요 취약 상권 1,570개 |

## 해결한 문제

- 좋아 보이는 매물을 추천하기보다, 먼저 사지 말아야 할 근거를 보여줍니다.
- 구 단위 리스크를 점수로만 끝내지 않고, 매물 검토 메모와 대체 후보 비교로 연결합니다.
- 분석 결과를 로컬 대시보드가 아니라 공개 가능한 웹사이트 경험으로 번역합니다.

## 핵심 기능

- `내 매물 검토`
  희망 매입가와 구를 기준으로 리스크 점수, 유형, 권장 액션을 확인합니다.
- `3분 진단`
  빠른 입력만으로 현재 매입 판단을 점검합니다.
- `후보 비교`
  검토 중인 후보 구를 나란히 비교합니다.
- `구별 리포트`
  서울 25개 구의 리스크 등급, 유형, 주요 근거를 확인합니다.
- `케이스 스터디`
  대표 위험 구를 실제 해석 흐름으로 보여줍니다.
...
| `src/redveil/pipelines/` | 수집, 가공, 스코어링, 사이트 payload 생성 |
| `docs/` | PRD, 전략, 사용자 여정, 검증 문서 |
| `dashboard/` | README 및 배포용 시각 자산 |
...
- `data/` 원천 파일은 용량 문제로 저장소에 포함하지 않았습니다.
- 공개 저장소는 코드, 문서, 사이트, 시각 자산 중심의 포트폴리오 버전입니다.
- 최신 거래 데이터와 상권 수요 데이터의 시점이 완전히 일치하지 않으므로, 실제 투자 판단의 1차 검토 도구로 보는 것이 적절합니다.
