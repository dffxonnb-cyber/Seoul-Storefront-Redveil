...
const captureEnabled = process.env.CAPTURE_V2_PORTFOLIO === "1";
const output = (name) => `test-results/v2-portfolio/${name}.png`;

test.skip(!captureEnabled, "Portfolio screenshots are generated only by the packaging workflow.");
...
test("capture desktop map dashboard", async ({ page }) => {
...
test("capture mobile map dashboard", async ({ page }) => {
...
test("capture property review result", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1080 });
  ...
  await page.locator("#exclusive-area").fill("43");
...
test("capture 3-minute assessment result", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1080 });
  await page.goto("/v2/assessment.html?district=11650");
  await page.locator("#asking-price").fill("2350");
...
test("capture candidate comparison", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1080 });
...
test("capture district report", async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1080 });
