...
| 메인 화면 | 구 리스크 검토 |
|------|------|
| ![Redveil 메인 화면](./dashboard/01_overview.png) | ![Redveil 구 리스크 검토 화면](./dashboard/02_district_risk_review.png) |

| 케이스 스터디 | 핵심 결과 |
|------|------|
| ![Redveil 케이스 스터디 화면](./dashboard/03_case_study.png) | 서울 25개 구 비교, 행정동 428개 커버, 수요 취약 상권 1,570개 분석, 거래 원천 데이터 12,074건 기반 리스크 판별 |
...
![Redveil 메인 화면](./dashboard/01_overview.png)

사용자가 서비스 목적과 현재 위험 구 요약, 주요 진입 버튼을 바로 이해할 수 있도록 설계했습니다.

### 구 리스크 검토 화면

![Redveil 구 리스크 검토 화면](./dashboard/02_district_risk_review.png)

구별 리스크 등급과 근거를 한 화면에서 읽고, 다음 검토로 이어질 수 있게 구성했습니다.

### 케이스 스터디 화면

![Redveil 케이스 스터디 화면](./dashboard/03_case_study.png)

숫자만 보여주는 대신 실제 검토 언어로 사례를 읽고, 왜 보류해야 하는지 납득할 수 있게 만들었습니다.
